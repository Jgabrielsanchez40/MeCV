...
router.get('/review/:id', async (req, res) => {
  ...
  const dataperson = await personnels.find({ user: req.params.id }).lean();
  const dataedu = await educations.find({ user: req.params.id }).lean();
  const datalang = await languages.find({ user: req.params.id }).lean();
  const dataskills = await skills.find({ user: req.params.id }).lean().sort({ skill: 'asc' });
  const dataknow = await knowings.find({ user: req.params.id }).lean();
  const dataexp = await experiences.find({ user: req.params.id }).lean().sort({ wfrom: 'desc' });
  res.render('data/alldata', { dataperson, dataedu, datalang, dataskills, dataknow, dataexp, datauser });
});
...
router.get('/pdf/:id', async (req, res) => {
  ...
  const lang = await languages.find({ user: req.params.id });
  const dskills = await skills.find({ user: req.params.id }).sort({ skill: 'asc' });
  const know = await knowings.find({ user: req.params.id }).sort({ knowings: 'asc' });
  const exp = await experiences.find({ user: req.params.id }).sort({ wfrom: 'desc' });
